...
public class Chunk {
    ...
    public Chunk(ArrayList<Vector3f> heights, int biomeID, int treeSeed) {
        x = (int) heights.get(heights.size()-1).x;
        z= (int) heights.get(heights.size()-1).y;
        Random r = new Random(x * z * treeSeed);
      ...
      int numTrees = (int)(r.nextDouble()*6+1);
      //numTrees = 0;
      ...
      for(int i=0;i<numTrees;i++) {
          if (i == 0) {
              int tX = (int) (r.nextDouble() * 14)+1;
              treeX[i] = tX;
              int tZ = (int) (r.nextDouble() * 14)+1;
              treeZ[i] = tZ;
          } else {
              int tX = (int) (r.nextDouble() * 14)+1;
              treeX[i] = tX;
              ...
              for (int j = i - 1; j >= 0; j--) {
                  if (treeX[i] == treeX[j] && treeZ[i] == treeZ[j]) {
                      treeX[i] = (int) (r.nextDouble() * 14)+1;
                      treeZ[i] = (int) (r.nextDouble() * 14)+1;
                      j = i - 1;
                  }
              }
          }
          ...
          if(quit < 100) {
              Tree t = new Tree(treeX[i] + x * 16, treeY[i] - h / 2, treeZ[i] + z * 16,ID[2],ID[3], r.nextDouble());
